...
def is_valid_direct_answer(response, direct_answer_format) -> bool:
    ...
    if response.count('<answer>') != 1 or response.count('</answer>') != 1:
        return False
    # 3). <resize> is not allowed!
    if '<resize>' in response:
        return False
    return True

def is_valid_resize_image(response, step_tool_call_format) -> bool:
...
def format_reward(predict_str_list: list, extra_info: dict = None):
    ...
    # All allowed formats
    direct_answer_format = r'.*<answer>.*</answer>$' 
    step_tool_call_format = r'.*<resize>$' 
    # 1-turn
    if conv_rounds == 1:
    ...
    # multi-turn
        ...
        for response in predict_str_list[:-1]:
            ...
            if "<resize>" in response:
                tool_call_count += 1
            # Call Function Tool
            if not is_valid_resize_image(response, step_tool_call_format):
